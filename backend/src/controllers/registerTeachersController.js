...
registerTeachersController.registerTeacher = async (req, res) => {
    const { name, lastName, email, password, phone, hireDate, isActive, isVerified, loginAttemps, timeOut } = req.body;
 
    try {
        const existTeacher = await teachersModel.findOne({ email });
        if (existTeacher) return res.status(400).json({ message: "Teacher already exists" });
 
        const passwordHashed = await bcryptjs.hash(password, 10);
        const randomNumber = crypto.randomBytes(3).toString("hex");
 
        const token = jsonwebtoken.sign(
            { randomNumber, name, lastName, phone, email, password: passwordHashed, isVerified },
            config.JWT.secret,
            { expiresIn: "15m" }
        );
...
registerTeachersController.verifyCode = async (req, res) => {
    try {
        ...
        const { randomNumber: storedCode, name, lastName, email, phone, password } = decoded;
